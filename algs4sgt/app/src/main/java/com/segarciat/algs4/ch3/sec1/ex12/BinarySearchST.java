package com.segarciat.algs4.ch3.sec1.ex12;
 ...
 * Implements a version of {@link edu.princeton.cs.algs4.BinarySearchST}
 * that uses a single array of <code>Item</code> values for key-value pairs,
 * instead of parallel array, as requested in Exercise 3.1.12.
 * I deviated from the requirement of relying on {@link edu.princeton.cs.algs4.Merge#sort(Comparable[]);}
 * and instead relied on {@link BinarySearchST#put(Comparable, Object)} because
 * it handles issues such as receiving a <code>null</code> key in the array from
 * the client, updating the key-value pairs if given duplicate keys, and keeping track
 * of the array size.
...
public final class BinarySearchST <Key extends Comparable<Key>, Value> {
    ...
    public void put(Key key, Value val) {
        if (key == null)
            throw new NullPointerException("cannot add a null key");
        // do not handle null value in this exercise.
        int i = rank(key);
        if (i < n && key.compareTo(items[i].key()) == 0) {
            items[i].val = val;
            return;
