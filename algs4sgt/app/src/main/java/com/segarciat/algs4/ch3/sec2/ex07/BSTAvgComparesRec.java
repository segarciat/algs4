 ...
 * recursive implementation of {@link BSTAvgComparesRec#avgCompares()}, which
 * computes the average number of compares required by a random hit in a given
 * BST.
 * @author Sergio E. Garcia Tapia
 */
public final class BSTAvgComparesRec<Key extends Comparable<Key>, Value>{
    ...
    public double avgCompares() {
        ...
        if (n == 0) {
            ...
            return 1 + (ipl(root, 0) / (n + 0.0));
        }
    }

    private int ipl(Node<Key, Value> node, int depth) {
        if (node == null) {
            return 0;
        } else {
            return depth
                    + ipl(node.left, depth + 1)
                    + ipl(node.right, depth + 1);
        }
    }
